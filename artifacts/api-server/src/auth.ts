  ...
  if (googleEnabled) {
    ...
    const getCallbackURL = () => {
      if (process.env.NODE_ENV === "production") {
        const prodDomain = (process.env.REPLIT_DOMAINS || "")
          .split(",")[0]
          ?.trim();
        return `https://${prodDomain || "quantitativereasoning101.xyz"}${CALLBACK_PATH}`;
      }
    ...
    const trustedHosts = new Set<string>(
      [
        ...
        "quantitativereasoning101.xyz",
        "www.quantitativereasoning101.xyz",
        "quantitative-reasoning-course.replit.app",
        "localhost:80",
      ]
        .filter(Boolean)
        .map((h) => h.toLowerCase())
