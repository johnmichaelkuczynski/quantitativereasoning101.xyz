...
// --- User & login-visit storage (all login-related DB access lives here) ---
const storage = {
...
export function setupAuth(app: Express) {
  ...
  app.use(
    session({
      store: pgStore,
      secret: process.env.SESSION_SECRET || "quantreason-studio-secret-key",
      resave: false,
  ...
  if (googleEnabled) {
    // Callback path is /api/auth/google/callback so it routes to this API
    // server (the shared proxy forwards only /api/* here). Register this
    // exact URI in the owner's Google Cloud Console OAuth client.
    ...
    const getCallbackURL = () => {
      if (process.env.NODE_ENV === "production") {
        const prodDomain = (process.env.REPLIT_DOMAINS || "")
          .split(",")[0]
          ?.trim();
        return `https://${prodDomain || process.env.REPLIT_DEV_DOMAIN || "localhost:80"}${CALLBACK_PATH}`;
      }
